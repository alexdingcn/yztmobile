...
function init() {
	...
	var len = UnSendoutDetailList.length;
	i = 0;
	for(; i < len;) {
	...
	$.each(UnSendoutDetailList, function(index, value) {
		strbig += '<div class="li">' +
			'<div class="opt"><i class="i-circle" onclick="select(this,' + index + ')"></i></div><a>' +
			...
			'<div class="spec">' + value.OrderOutDetailInfo.ValueInfo + '</div>' +
			'<div class="acolor">' + "已发货:" + value.OutNum + '</div></a>' +
			'<div class="wait">' +
	...
	$('#confirm').on('click', function() {
		...
		if(isDelivery) {
			...
			window.location.href = 'order_logistics_editor.html?'+Math.random();
		} else {
			$.tips({
				content: '请选择发货商品',
				stayTime: 3000,
